...
print('######################################## print data ####################################')
imu_f = data['imu_f']
print('acc data', imu_f.data)
print('Imu timestamp:', imu_f.t)
print('accel shape', np.shape(imu_f.data))


imu_w = data['imu_w']
print('angular data', imu_w.data)
print('Imu timestamp:', imu_w.t)
print('angular shape', np.shape(imu_w.data))

gnss = data['gnss']
print('gnss data', gnss.data)
print('gnss timestamp', gnss.t)
print('gnss shape', np.shape(gnss.data))

lidar = data['lidar']
print('lidar data', lidar.data)
print('lidar timestamp', lidar.t)
print('lidar shape', np.shape(lidar.data))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p + delta_t * v + 0.5 * delta_t**2 * (C_ns @ imu_f.data[k-1] + g)
    ...
    q_check = Quaternion(axis_angle=imu_w.data[k-1] * delta_t).quat_mult_left(q)
    ...
    if time_stamp in gnss_time_set:
        gnss_i = list(gnss_time_set).index(time_stamp)
        p_check, v_check, q_check, P_cov_check = measurement_update(var_gnss, P_cov_check, gnss.data[gnss_i], p_check, v_check, q_check)
    
    
    if time_stamp in lidar_time_set:
        lidar_i = list(lidar_time_set).index(time_stamp)
